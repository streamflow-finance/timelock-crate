...
#[tokio::test]
async fn test_not_transferable_sender() -> Result<()> {
    let strm_key = Pubkey::from_str(STRM_TREASURY).unwrap();
    let metadata_kp = Keypair::new();
    let alice = Account { lamports: sol_to_lamports(10.0), ..Account::default() };
    let bob = Account { lamports: sol_to_lamports(10.0), ..Account::default() };
    ...
    let is_err = transaction.is_err();
    assert_eq!(is_err, !transferable_by_sender);

    Ok(())
}

#[tokio::test]
async fn test_not_transferable_recipient() -> Result<()> {
    let strm_key = Pubkey::from_str(STRM_TREASURY).unwrap();
    let metadata_kp = Keypair::new();
    let alice = Account { lamports: sol_to_lamports(10.0), ..Account::default() };
    let bob = Account { lamports: sol_to_lamports(10.0), ..Account::default() };
    ...
    let transaction = tt.bench.process_transaction(&[transfer_ix_bytes], Some(&[&bob])).await;
    let is_err = transaction.is_err();
    assert_eq!(is_err, !transferable_by_recipient);

    Ok(())
}

#[tokio::test]
async fn test_transfer_not_signer() -> Result<()> {
    let strm_key = Pubkey::from_str(STRM_TREASURY).unwrap();
    let metadata_kp = Keypair::new();
    let alice = Account { lamports: sol_to_lamports(10.0), ..Account::default() };
    let bob = Account { lamports: sol_to_lamports(10.0), ..Account::default() };
    ...
    let transfer_ix_bytes = Instruction::new_with_bytes(
        ...
        &transfer_ix.try_to_vec()?,
        vec![
            AccountMeta::new(bob.pubkey(), false),    // sender
            AccountMeta::new(alice.pubkey(), false),  // New recipient
            AccountMeta::new(alice_ass_token, false), // New recipient token account
            AccountMeta::new(metadata_kp.pubkey(), false),
            AccountMeta::new_readonly(strm_token_mint.pubkey(), false),
    ...
    let transaction = tt.bench.process_transaction(&[transfer_ix_bytes], Some(&[])).await;
    let is_err = transaction.is_err();
    assert!(is_err);
...
#[tokio::test]
async fn test_transfer_myself_fails() -> Result<()> {
    let strm_key = Pubkey::from_str(STRM_TREASURY).unwrap();
    let metadata_kp = Keypair::new();
    let alice = Account { lamports: sol_to_lamports(10.0), ..Account::default() };
    let bob = Account { lamports: sol_to_lamports(10.0), ..Account::default() };
...
#[tokio::test]
async fn test_transfer_self_stream_to_other_success() -> Result<()> {
    let strm_key = Pubkey::from_str(STRM_TREASURY).unwrap();
    let metadata_kp = Keypair::new();
    let alice = Account { lamports: sol_to_lamports(10.0), ..Account::default() };
    let bob = Account { lamports: sol_to_lamports(10.0), ..Account::default() };
    ...
    let transfer_ix_bytes = Instruction::new_with_bytes(
        ...
        vec![
            AccountMeta::new(alice.pubkey(), true), // sender
            AccountMeta::new(bob.pubkey(), false),  // New recipient
            AccountMeta::new(bob_ass_token, false), // New recipient token account
            AccountMeta::new(metadata_kp.pubkey(), false),
            AccountMeta::new_readonly(strm_token_mint.pubkey(), false),
            AccountMeta::new_readonly(rent::id(), false),
            AccountMeta::new_readonly(spl_token::id(), false),
